...
const ProductCard = ({ product }: { product: Product }) => {
  ...
  return (
    ...
    >
      {product.tag && (
        <span className="absolute right-3 top-3 z-10 rounded-full bg-primary px-3 py-1 text-xs font-semibold text-primary-foreground shadow-lg shadow-primary/30">
          {product.tag}
        </span>
      )}
      ...
      <div className="flex flex-1 flex-col p-6">
        ...
        <div className="mt-4 flex flex-wrap gap-1.5">
          {product.features.slice(0, 3).map((f) => (
            <span key={f} className="rounded-full bg-secondary/80 px-2.5 py-1 text-[11px] text-muted-foreground">
              {f}
            </span>
          ))}
        </div>
